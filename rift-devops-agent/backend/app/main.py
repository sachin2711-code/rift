...
#     # Create branch name
#     branch_name = f"{request.team_name.upper().replace(' ', '_')}_{request.team_leader_name.upper().replace(' ', '_')}_AI_Fix"
...
# async def execute_agent_run(run_id: str, request: AgentRunRequest, branch_name: str):
#     """
...
#     """Get the complete results of a run"""
#     if run_id not in active_runs:
#         raise HTTPException(status_code=404, detail="Run not found")
...
#     """Get all fixes applied in a run"""
#     if run_id not in active_runs:
#         raise HTTPException(status_code=404, detail="Run not found")
...
#     """Get the diff for a specific fix"""
#     if run_id not in active_runs:
#         raise HTTPException(status_code=404, detail="Run not found")
...
#         filename=f"rift_report_{run_id}.pdf"
#     )


# @app.get("/api/v1/runs/{run_id}/tree")
# async def get_repository_tree(run_id: str):
#     """Get the repository file tree with error annotations"""
#     if run_id not in active_runs:
#         raise HTTPException(status_code=404, detail="Run not found")
    
#     run_data = active_runs[run_id]
    
#     # Get tree from memory service
#     tree = await memory_service.get_repository_tree(run_id)
    
#     # Annotate with fix information
#     fixes = run_data.get("fixes", [])
#     for fix in fixes:
#         file_path = fix.get("file_path", "")
...
# ):
#     """Get learned bug patterns from memory"""
...
#             "score_difference": (run2.get("score", {}).get("total_score", 0) - 
...
#     """Get bug heatmap data for visualization"""
#     if run_id not in active_runs:
#         raise HTTPException(status_code=404, detail="Run not found")
...
# os.makedirs("/tmp/rift-results", exist_ok=True)
# app.mount("/results", StaticFiles(directory="/tmp/rift-results"), name="results")
